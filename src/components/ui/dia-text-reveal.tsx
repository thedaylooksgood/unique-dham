...
> {
  ...
  fixedWidth?: boolean
  /**
   * If true, the text reveals and then hides itself by sweeping back.
   */
  revealAndHide?: boolean
  /**
   * Delay before hiding after reveal, in seconds.
   */
  hideDelay?: number
}
...
export function DiaTextReveal({
  ...
  playRef.current = () => {
    const { duration, delay, repeat, repeatDelay, texts, revealAndHide, hideDelay } = optsRef.current
      ...
      onComplete() {
        if (revealAndHide) {
          timerRef.current = setTimeout(() => {
            const hideControls = animate(sweepPos, SWEEP_START, {
              duration,
              ease: sweepEase,
              onComplete() {
                if (!repeat) return
            ...
            stopRef.current = () => hideControls.stop()
          }, hideDelay * 1000)
          return
        }
